...
export interface Message {
  ...
  inputTokens?: number;
  /**
   * Идемпотентность отправки. Клиент генерирует id один раз на реплику и
   * повторяет его при ретраях, поэтому повторная отправка не создаёт вторую
   * копию, а получает уже готовый ответ. Только у role="user"; у старых
   * записей отсутствует.
   */
  clientId?: string;
}
